...
#define FILAGUI_RESOURCES_UIBLIT_DATA 50
#define FILAGUI_RESOURCES_UIBLIT_DATA 50
...
ImGuiHelper::ImGuiHelper(Engine* engine, filament::View* view, const Path& fontPath,
    ...
    std::ifstream file("output.filemat", std::ios::binary | std::ios::ate);
    ...
    mMaterial2d = filament::Material::Builder().package(buffer.data(), buffer.size())
        .build(*engine);

    filament::MaterialInstance* matInstance = mMaterial2d->createInstance();
